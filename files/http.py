...
class HttpDownloadHandler(object):
    ...
    def _connect(self, factory):
        host, port, bind_address = factory.host, factory.port, factory.bind_address
        if factory.scheme == 'https':
            if ssl_supported:
                return reactor.connectSSL(host, port, factory, \
                        self.ClientContextFactory(), bindAddress=bind_address)
            raise NotSupported("HTTPS not supported: install pyopenssl library")
        else:
            return reactor.connectTCP(host, port, factory, bindAddress=bind_address)
